...
ODD_TARGETS = (-9, -7, -5, -3, -1, 1, 3, 5, 7, 9)
ODD_INDEX = {v: i for i, v in enumerate(ODD_TARGETS)}

# True=feasible, False=infeasible, packed as 10-bit rows.
# Each mod table is a list of 10 integers (rows=power, cols=stamina),
# where bit i (0..9) corresponds to ODD_TARGETS[i] in the column.
FEAS_MOD_TABLE = [
...
PAIRS_BY_PRIORITY_PREFER_POWER = sorted(
    ((p + s, p, s) for p in ODD_TARGETS for s in ODD_TARGETS),
    reverse=True,
    key=lambda t: (t[0], t[1], t[2]),
)

PAIRS_BY_PRIORITY_PREFER_STAMINA = sorted(
    ((p + s, p, s) for p in ODD_TARGETS for s in ODD_TARGETS),
    reverse=True,
